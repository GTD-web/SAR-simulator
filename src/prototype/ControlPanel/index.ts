...
export class ControlPanelManager {
  ...
  private flyToSatelliteStateCheckInterval: ReturnType<typeof setInterval> | null = null;
  ...
  private createControlPanel(viewer?: any, options?: ControlPanelOptions): void {
    ...
    // 위성으로 이동 버튼 (화면 하단 중앙) - 위성 고정 상태 표시
    ...
    this.updateFlyToSatelliteButtonState();
    this.flyToSatelliteStateCheckInterval = setInterval(
      () => this.updateFlyToSatelliteButtonState(),
      400
    );
   ...
   * 위성 고정 상태 표시 갱신
   */
  private updateFlyToSatelliteButtonState(): void {
    if (!this.flyToSatelliteButton || !this.viewer) return;

    const busEntity = this.satelliteSettings?.getBusPayloadManager()?.getBusEntity();
    const isTracking = !!busEntity && this.viewer.trackedEntity === busEntity;

    if (isTracking) {
      this.flyToSatelliteButton.classList.add('tracking');
      this.flyToSatelliteButton.title = '카메라 위성 고정 중 (클릭 시 해제)';
      this.flyToSatelliteButton.setAttribute('aria-label', '카메라 위성 고정 중');
    } else {
      this.flyToSatelliteButton.classList.remove('tracking');
      this.flyToSatelliteButton.title = '위성으로 이동';
   ...
   */
  cleanup(): void {
    if (this.flyToSatelliteStateCheckInterval) {
      clearInterval(this.flyToSatelliteStateCheckInterval);
      this.flyToSatelliteStateCheckInterval = null;
    }
